...
const noSave = computed(() => {
    return (nameserverIP.value.length < 6 || isSplitDNS.value && (!restrictDomain.value || restrictDomain.value.length == 0))
})

watch(() => nameserverIP.value, (newV) => {
    invalidNS.value = false
    existNS.value = false
    nameserverIP.value = nameserverIP.value.toLowerCase().replace(/[^\.:0-9a-f]/ig, '')
})

watch(() => restrictDomain.value, (newV) => {
...
function SaveEditedNS() {
    if (nameserverIP.value == props.OriResolver && (restrictDomain.value == props.OriDomain && isSplitDNS.value || props.OriDomain == "" && !isSplitDNS.value)) {
        emit("close")
    }
    ...
    if (isSplitDNS.value && nameserverIP.value != props.OriResolver && restrictDomain.value == props.OriDomain) {
        //splitDNS修改服务器ip地址
        reqData = props.currentDNS
    ...
    if (isSplitDNS.value && restrictDomain.value != props.OriDomain) {
        ...
        for (var i in reqData["domains"]) {
            if (reqData["domains"][i] == restrictDomain.value) {
                ...
                if (!reqData["routes"][restrictDomain.value]) reqData["routes"][restrictDomain.value] = []
...
onMounted(() => {
    ...
    if (restrictDomain.value != "")
        isSplitDNS.value = true
})
...
<template>
    <div @click.self="$emit('close')" class="fixed overflow-y-auto inset-0 py-8 z-30 bg-gray-900 bg-opacity-[0.07]"
        ...
        <div class="bg-white rounded-lg relative p-4 md:p-6 text-gray-700 max-w-lg min-w-[19rem] my-8 mx-auto w-[97%] shadow-2xl"
            ...
            <form @submit.prevent="">
                <div class="mt-8 mb-8"><label class="font-medium text-gray-900 block mb-1"
                        for="nameserver">域名服务器</label>
                    <p class="text-sm text-gray-600 mt-2">使用这个IPv4或IPv6地址来解析域名</p>
                    <div class="relative mt-2">
                        <input v-model="nameserverIP"
                            :class="{ 'border-stone-200': !invalidNS && !existNS, 'border-red-400': invalidNS || existNS }"
                            ...
                            id="nameserver" type="text" placeholder="1.2.3.4" autocapitalize="off" autocomplete="off">
                    </div>
                    <p v-if="invalidNS" class="text-sm text-red-400 mt-1">不合法，只允许IPv4或IPv6地址</p>
                    <p v-if="existNS" class="text-sm text-red-400 mt-1">该域名解析服务器条目已存在</p>
                </div>
                <div class="sm:flex justify-between items-center">
                    <div>
                        <div class="flex items-center"><label class="font-medium text-gray-900" for="restrict">分离
                                DNS</label><span data-state="closed">
                                <div
                        ...
                        <p class="text-sm text-gray-600 hidden sm:block">此域名服务器将只用于部分域名解析</p>
                    </div>
                    <span data-state="closed">
                        <input v-model="isSplitDNS" id="restrict" type="checkbox"
                ...
                <div v-if="isSplitDNS" class="mt-4">
                    <label class="font-medium text-gray-900 block mb-1" for="searchdomain">检索域</label>
                    <div class="relative mt-2">
                        <input v-model="restrictDomain"
                            ...
                            id="searchdomain" type="text" placeholder="example.com" autocapitalize="off"
                            autocomplete="off">
                    </div>
                    <p class="text-sm text-gray-600 mt-2">只用匹配此后缀的单一标签或完全限定查询将使用这个域名服务器</p>
                </div>
                <footer class="flex mt-10 justify-end space-x-4">
                    <button @click="$emit('close')"
                        class="btn border border-base-300 hover:border-base-300 bg-base-200 hover:bg-base-300 text-black h-9 min-h-fit">取消</button>
                    <button @click="SaveEditedNS"
                        class="btn border-0 bg-blue-500 hover:bg-blue-900 disabled:bg-blue-500/60 text-white disabled:text-white/60 h-9 min-h-fit"
                        :disabled="noSave">保存</button>
                </footer>
            </form>
            <button @click="$emit('close')"
