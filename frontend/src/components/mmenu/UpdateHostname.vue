      ...
      if (response.data["status"] == "success") {
        if (response.data["data"]["name"] == wantName.value) {
          emit(
            ...
            response.data["data"]["automaticNameMode"],
            true
          );
        } else if (reqName === "") {
          setName.value = response.data["data"]["name"];
          emit(
            "update-done",
            response.data["data"]["name"],
            response.data["data"]["automaticNameMode"],
    ...
    >
      ...
      <form @submit.prevent="$emit('confirm')">
        <p class="text-gray-700 mb-6">
          这个名称显示在控制台、客户端，以及生成对应的「<strong>幻域</strong>」名称
        </p>
        <div class="flex items-center">
          ...
          />
          <label for="auto-generate-checkbox" class="font-medium ml-3"
            >由设备OS主机名生成</label
          >
        </div>
        ...
        <div class="flex mb-2">
          <div class="relative w-full z-30">
            <input
              ...
              type="text"
              :disabled="inputBlocking || setAutogen"
              id="machine-name"
            />
          </div>
        </div>

        <p v-if="wantNameWithHash" class="text-sm text-red-500 mb-2">
          名称 “{{ wantName }}” 已有设备占用 , 因此设备被自动命名为 “{{ givenName }}”.
        </p>
        <p v-if="wantNameOccupied" class="text-sm text-red-500 mb-2">
          名称 “{{ wantName }}” 已有设备占用
        </p>
        <p v-if="wrongSetName" class="text-sm text-red-500 mb-2">
          名称不能为空，只能是字母数字和连接线组成，且不能以连接线开头结尾
        </p>

        <div v-if="!valueChanged" class="text-sm text-gray-600">
          这个设备可以通过「幻域」使用
          <code class="text-xs break-words bg-gray-200 px-1 rounded">{{
            givenName
          }}</code>
          进行访问
        </div>
        <div v-if="valueChanged" class="text-sm text-gray-600">
          这个设备可以通过「幻域」使用
          <code class="text-xs break-words bg-gray-200 px-1 rounded">{{ setName }}</code>
          进行访问。
          <code class="text-xs break-words bg-gray-200 px-1 rounded">{{
            givenName
          }}</code>
        ...
        <footer class="flex mt-10 justify-end space-x-4">
          ...
          <button
            :disabled="inputBlocking || (!setChanged && !valueChanged)"
            @click="updateName"
