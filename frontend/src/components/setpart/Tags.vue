...
function doRemoveTag() {
  axios
    .delete("/admin/api/tags/" + wantRemoveTag.value, {})
    .then(function (response) {
      if (response.data["status"] == "success") {
        var tmpTagOwners = []
...
<template>
  <div class="flex-1">
    ...
    <div class="text-gray-600 mt-3 mb-10">
      ...
      <p class="mt-2">使用标签可以很方便地设置服务应用类节点。它们通常不应归属于某个或某些用户，但代表部分用户可以访问到的提供业务服务的节点。</p>
      <p class="mt-2">一个标签的标签管理员意味着他可以将他所登录的节点打上对应的标签，从而由标签管理员负责某服务节点的部署配置。</p>
    </div>
    <div class="mt-10">
      <div class="flex justify-between items-center mt-16">
        <div>
          <h3 class="text-xl font-semibold tracking-tight">现有标签</h3>
          <p class="text-gray-600">设置的标签可在设备管理、授权密钥中分配，可在ACL中使用从而设置用户对服务的访问管理 </p>
        </div>
        <button @click="showSetTag"
      ...
      <div v-if="!tagOwners || tagOwners.length == 0" class="rounded-md border border-stone-200 mt-4 bg-stone-50 p-6">
        <div class="flex justify-center">
          <div class="w-full text-center max-w-xl text-gray-500">你还没有任何标签</div>
        </div>
      </div>

      <table v-if="tagOwners && tagOwners.length > 0" class="block border box-border rounded-lg mt-4 tb">
          ...
          <template v-for="tag, i in tagOwners">
            <tr :class="{ 'border-t': i > 0 }"
              ...
              <td class="flex shrink-0 py-2 w-36">
                <pre class="text-sm truncate leading-6 font-semibold"><code>{{ tag.tagName.substring(4) }}</code></pre>
              </td>
              ...
              <td
                ...
                <button @click="toRemoveTag(tag.tagName.substring(4))" type="button">删除…</button>
              </td>
            </tr>
          </template>
        </tbody>
  ...
  <Teleport to="body">
    <!-- 创建Tag提示框显示 -->
    <SetTag v-if="setTagShow" @added-tag="createTagDone" @close="setTagShow = false"></SetTag>
    <!-- 删除Tag提示框显示 -->
    <template v-if="DeleteTagShow">
      <div @click.self="DeleteTagShow = false"
        ...
        <div
          ...
          <form @submit.prevent="doRemoveTag">
            <p class="text-gray-700 mb-4">删除此标签前请确保已经没有节点或ACL规则使用此标签</p>
            <footer class="flex mt-10 justify-end space-x-4">
              <button @click="DeleteTagShow = false"
