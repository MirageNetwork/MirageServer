...
<template>
  <div>
    <div class="pt-4 pb-10 border-b border-gray-200 max-w-xl mx-auto">
      <div v-if="downloadDetails.android.primary == ''" class="text-center">
        <button
          disabled
      ...
      <div
        v-if="downloadDetails.android.primary != ''"
        class="md:flex items-center justify-center"
      >
        <a
          :href="downloadDetails.android.primary"
          class="border overflow-hidden rounded-lg block max-w-[10rem] mx-auto md:mx-0 mb-8 md:mb-0"
          data-tooltip="使用你手机的相机扫描QR码下载蜃境客户端"
        >
          <vue-qr :text="downloadDetails.android.primary" :size="400"></vue-qr>
        </a>
        <div class="max-w-[18rem] md:ml-6 text-center md:text-left mx-auto md:mx-0">
          <p class="mb-3 text-gray-600">扫描二维码或用下方链接下载蜃境Android版</p>
          <a :href="downloadDetails.android.primary"
            ><button class="btn btn-sm border-0 bg-blue-600 hover:bg-blue-700 text-white">
              下载 <span class="hidden md:inline">蜃境客户端</span> Android版
            </button>
          </a>
          <p class="mt-2 text-sm text-gray-600">需要Android 6及以上系统</p>
        </div>
      </div>
    </div>
    <div class="mx-auto max-w-xl">
      <header class="my-6 pt-4">
        <h2 class="text-xl font-medium">在Android系统上使用蜃境</h2>
      </header>
      <div class="Markdown mb-4">
        <ol>
          <li>下载安装并打开蜃境Android App</li>
