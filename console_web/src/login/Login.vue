...
<template>
  ...
  <form v-if="IDPs.includes('Dingtalk')" method="POST" class="mb-3">
    <input type="hidden" name="provider" value="Dingtalk" />
    ...
    <button type="submit"
      ...
      <svg t="1679382174674" class="icon ml-8 mr-3" viewBox="0 0 1024 1024" version="1.1"
  ...
  <form v-if="IDPs.includes('Github')" method="POST" class="mb-3">
    ...
    <button type="submit"
      class="btn btn-outline rounded-md shadow border-stone-300 hover:border-stone-400 hover:bg-transparent text-black hover:text-black h-10 min-h-fit w-64 justify-start">
      <svg t="1679387527759" class="icon ml-8 mr-3" viewBox="0 0 1024 1024" version="1.1"
        xmlns="http://www.w3.org/2000/svg" p-id="3364" width="20" height="20">
        <path
  ...
  <form v-if="IDPs.includes('Google')" method="POST" class="mb-3">
    ...
    <button type="submit"
      class="btn btn-outline rounded-md shadow border-stone-300 hover:border-stone-400 hover:bg-transparent text-black hover:text-black h-10 min-h-fit w-64 justify-start">
      <svg t="1679449475826" class="icon ml-8 mr-3" viewBox="0 0 1024 1024" version="1.1"
        xmlns="http://www.w3.org/2000/svg" p-id="4669" width="20" height="20">
        <path
  ...
  <form v-if="IDPs.includes('Apple')" method="POST" class="mb-3">
    ...
    <button type="submit"
      class="btn btn-outline rounded-md shadow border-stone-300 hover:border-stone-400 hover:bg-transparent text-black hover:text-black h-10 min-h-fit w-64 justify-start">
      <svg t="1679468518353" class="icon ml-8 mr-3" viewBox="0 0 1024 1024" version="1.1"
        xmlns="http://www.w3.org/2000/svg" p-id="1724" width="20" height="20">
        <path
  ...
  <form v-if="IDPs.includes('Ali')" method="POST" class="mb-3">
    ...
    <button type="submit"
      class="btn btn-outline rounded-md shadow border-stone-300 hover:border-stone-400 hover:bg-transparent text-black hover:text-black h-10 min-h-fit w-64 justify-start">
      <svg t="1674566173646" class="ml-8 mr-3" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
  ...
  <form v-if="IDPs.includes('WeChat') && !showWXMiniCode" @submit.prevent="startWXScan" class="mb-3">
  ...
  <div v-if="IDPs.includes('Ali') && !showRegister && !showRegSuccess && !showWXMiniCode"
    class="mt-3 mb-2 text-stone-500 text-xs">
    还没有账号？
  </div>
  <Register v-if="IDPs.includes('Ali')" :wantMeClose="closeRegister" :show="showRegister" @close="doCloseRegister"
    @reg-done="doRegSuccess">
  </Register>

  <Transition v-if="IDPs.includes('Ali')" enter-from-class="opacity-0"
  ...
  <WxQR v-if="showWXMiniCode" :show="showWXMiniCode" :wxminicode="WXMiniCode" :checkcode="WXMiniCheck"
    @close="closeWXMini">
  </WxQR>

  <footer class="mt-10 text-sm text-stone-600">
    <p>
      <strong>不会用？</strong> 了解更多请发邮件至<a class="underline" href="mailto:gps949@nopkt.com?subject=[关于蜃境]">
