  ...
  axios
    ...
    .then(function (response) {
      ...
      if (response.data["status"] == "success") {
        ...
        toastMsg.value = "获取密钥失败:" + response.data["status"].substring[6];
        toastShow.value = true;
      }
    })
    .catch(function (error) {
      // 处理错误情况
      toastMsg.value = "获取密钥失败:" + error;
      toastShow.value = true;
    });
}
  ...
  axios
    ...
    .then(function (response) {
      if (response.data["status"] == "success") {
        ...
        toastMsg.value = "删除密钥失败:" + response.data["status"].substring[6];
        toastShow.value = true;
      }
    })
    .catch(function (error) {
      // 处理错误情况
      toastMsg.value = "删除密钥失败:" + error;
      toastShow.value = true;
    });
}
...
<template>
  <div class="flex-1">
    ...
    <div class="text-gray-600 mt-3 mb-10">
      ...
      <p class="mt-2">
        您个人设备的私钥<strong>不</strong>在此管理，它们永远是<strong>私有</strong>并存在于您的设备上，<strong>不</strong>向蜃境披露
      </p>
    </div>
    <div class="mt-10">
      ...
      <div
        v-if="!authKeys || authKeys.length == 0"
      ...
      >
        ...
        <tbody class="block">
          <template v-for="(authKey, id) in authKeys">
            <tr
              @click="authKey.expand = authKey.expand ? false : true"
              class="group flex border-stone-200 cursor-pointer hover:bg-gray-50 pl-8 pr-4 border-b-0"
              :class="{
                'border-t': id > 0,
                'lg:cursor-auto':
                  !authKey.authkey.tags || authKey.authkey.tags.length == 0,
              }"
            >
              <td class="flex shrink-0 py-2 w-36">
                ...
                >
                  ...
                  >
                    <polyline
                      :points="authKey.expand ? '6 9 12 15 18 9' : '9 18 15 12 9 6'"
                    ></polyline>
                  </svg>
                </div>
                <pre
                  class="text-sm truncate leading-6 font-semibold"
                ><code>{{ authKey.id }}</code></pre>
              </td>
              <td class="hidden shrink-0 py-2 lg:block w-40">
                <span data-state="closed"
                  ><span class="cursor-default">
                    {{ authKey.created.split(" ")[0] }}</span
                  ></span
                >
              </td>
              <td class="hidden shrink-0 py-2 lg:block w-40">
                <span data-state="closed"
                  ><span class="cursor-default">
                    {{ authKey.expiry.split(" ")[0] }}</span
                  ></span
                >
              </td>
              <td class="flex-1 shrink-0 py-2 min-w-0">
                {{
                  (authKey.authkey.reusable == true ? "可重用" : "一次性") +
                  (authKey.authkey.ephemeral == true ? ",自熄" : "")
                }}
              </td>
              <td
                class="w-20 shrink-0 py-2 text-right text-red-400 cursor-pointer pointer-events-auto hover:text-red-600"
            ...
            <tr v-if="authKey.expand" class="border-b-0 flex border-stone-200">
              <td class="shrink-0 p-0">
                <div
                  class="flex-col px-8 pt-0.5 pb-2"
                  :class="{
                    'lg:hidden':
                      !authKey.authkey.tags || authKey.authkey.tags.length == 0,
                  }"
  ...
  <Teleport to="body">
    ...
    <GenAuthKey
      ...
      @added-authkey="doAddAuthkey"
      @close="genAuthKeyShow = false"
    ></GenAuthKey>
    <!-- 注销授权密钥提示框显示 -->
    <template v-if="RevokeAuthKeyShow">
      <div
        @click.self="RevokeAuthKeyShow = false"
        ...
        >
          ...
          <form @submit.prevent="doRevokeAuthKey">
            <p class="text-gray-700 mb-4">
              注销此密钥<strong>并不会</strong>注销已使用此密钥进行授权的设备，但将阻止之后继续使用此密钥注册新设备。
            </p>
            <footer class="flex mt-10 justify-end space-x-4">
              <button
                @click="RevokeAuthKeyShow = false"
