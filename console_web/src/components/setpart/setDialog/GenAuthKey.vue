  ...
  axios
    ...
    .then(function (response) {
      if (response.data["status"] == "success") {
        ...
        toastMsg.value = "生成密钥失败：" + response.data["status"].substring[6];
        toastShow.value = true;
      }
    })
    .catch(function (error) {
      toastMsg.value = "生成密钥失败：" + error;
      toastShow.value = true;
    })
    .then(function () {
    ...
    >
      ...
      <form @submit.prevent="doKeyGen" v-if="!KeyGened">
        ...
        <div class="mt-4">
          ...
          <p class="text-sm text-gray-500">
            该授权密钥有效期天数。这个并不会影响使用该密钥授权的设备自身的设备密钥有效期限
          </p>
          <div class="flex mt-4">
            <div class="relative">
              <input
        ...
        <div
          class="border-t font-medium text-sm text-gray-500 tracking-wider uppercase mt-6 mb-1 pt-6"
        >
          设备设置
        </div>
        ...
        <div class="flex justify-between mt-4">
          <div>
            ...
            <p class="text-sm text-gray-500">
              使用该密钥授权的设备会自动添加标签。这同时会使该设备自身密钥过期被禁用
            </p>
          </div>
          <div class="ml-6">
            <input
              @change="addedTags.splice(0)"
              :disabled="inputBlocking"
              v-model="isTagged"
              type="checkbox"
              class="toggle"
        ...
        <div
          v-if="isTagged && addedTags.length == 0"
      ...
      <form @submit.prevent="copyGenedKey" v-if="KeyGened">
        <p class="text-gray-700 mb-3">
          关闭前请确保您已复制下面新生成的密钥，它之后将不会再次完整展示
        </p>
        <div
        ...
        <div
          class="flex overflow-hidden rounded-md py-3 px-4 gap-2 text-sm bg-stone-50 text-gray-600 border border-stone-200"
          role="alert"
        >
          <div class="pt-px">
            <svg
          ...
          <div class="w-full">
            该密钥将在
            {{ genedKey.expiry.split(" ")[0] }}
            过期，之后您要想继续使用授权密钥需要重新生成
          </div>
        </div>
        <footer class="flex mt-10 justify-end space-x-4">
          <button
