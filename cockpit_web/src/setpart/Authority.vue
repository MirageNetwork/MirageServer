...
function setDingtalk() {
  axios
    .post("/cockpit/api/setting/general", {
      state: "set-dingtalk",
      Dingtalk: Dingtalk.value,
    })
    .then(function (response) {
      // 处理成功情况
      if (response.data["status"] == "success") {
        Dingtalk.value = response.data["data"]["dingtalk"];
        toastMsg.value = "已更新钉钉设置";
        toastShow.value = true;
      } else {
        toastMsg.value = response.data["status"].substring(6);
        toastShow.value = true;
...
<template>
    ...
    <div class="mt-6 space-y-6">
      ...
      <div>
        <header class="max-w-sm flex">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="28" preserveAspectRatio="xMinYMin meet"
            viewBox="0 0 256 256" id="elasticsearch">
            <path fill="#FFF"
          ...
          <div class="w-full flex justify-end">
            <button :disabled="!isValidURL(ESURL) || ESKey == ''" @click="setES"
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="ESURL" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="ESKey" />
        </div>
      </div>
      <!---->
      <div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="WXScanURL" />
          <button :disabled="!isValidURL(WXScanURL)" @click="setWXScanURL"
      ...
      <div v-if="enableAli">
        <header class="max-w-sm flex">
          ...
          <div class="w-full flex justify-end">
            <button :disabled="SMS.id == '' || SMS.key == '' || SMS.sign == '' || SMS.template == ''
              " @click="setSMS"
              class="btn border-0 bg-blue-500 hover:bg-blue-900 disabled:bg-blue-500/60 text-white disabled:text-white/60 h-7 min-h-fit">
              保存
            </button>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="SMS.id" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="SMS.key" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="SMS.sign" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="SMS.template" />
        </div>
      </div>
      <!---->
      <div v-if="enableAli">
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="IDaaS.app" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="IDaaS.id" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="IDaaS.key" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="IDaaS.instance" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="IDaaS.org" />
        </div>
      </div>
      <!---->
      <div v-if="enableAli">
        <header class="max-w-sm flex">
          ...
          <div class="w-full flex justify-end">
            <button :disabled="!isValidURL(OIDC.issuer) || OIDC.id == '' || OIDC.key == ''" @click="setOIDC"
              class="btn border-0 bg-blue-500 hover:bg-blue-900 disabled:bg-blue-500/60 text-white disabled:text-white/60 h-7 min-h-fit">
              保存
            </button>
          </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="OIDC.issuer" />
        </div>
        <p class="mt-3 text-gray-600">Client ID</p>
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="OIDC.id" />
        </div>
        <p class="mt-3 text-gray-600">Client Key</p>
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="OIDC.key" />
        </div>
        <div class="mt-3 flex flex-row items-center max-w-sm">
          ...
          <div
            ...
            <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="newScope" />
            <button :disabled="newScope.trim() == ''" @click="addScope"
        ...
        <div class="mt-3 flex flex-row items-center max-w-sm">
          ...
          <div
            ...
            <input class="border-r bg-stone-50 outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis"
              v-model="newExtra.k" />
            <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="newExtra.v" />
            <button :disabled="!newExtra.k ||
              !newExtra.v ||
              newExtra.k.trim() == '' ||
              newExtra.v.trim() == ''
              " @click="addExtra"
        ...
        <div class="flex flex-wrap gap-2 p-1 mt-2 max-w-sm">
          <span v-for="(v, k) in OIDC.extra">
            <div
              ...
              <span class="pl-2 pr-1 py-1 text-gray-500 bg-stone-200 rounded-l-full h-full">{{ k }}</span>
      ...
      <div>
        <header class="max-w-sm flex mt-4">
          <svg t="1679382174674" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2682"
            width="112" height="28">
          ...
          <div @click="microsoftHelpShow = true" class="tooltip flex items-center text-stone-300" data-tip="点击查看帮助">
            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2.35" stroke-linecap="round" stroke-linejoin="round" class="ml-1">
          ...
          <div class="w-full flex justify-end">
            <button :disabled="Dingtalk.client_id == '' || Dingtalk.client_secret == ''" @click="setDingtalk"
              class="btn border-0 bg-blue-500 hover:bg-blue-900 disabled:bg-blue-500/60 text-white disabled:text-white/60 h-7 min-h-fit">
              保存
            </button>
          </div>
        ...
        <p class="mt-3 text-gray-600">Client ID</p>
        <div :class="{
            'border-red-500 hover:border-red-700': Dingtalk.client_id == '',
            'border-stone-200 hover:border-stone-400': Dingtalk.client_id != '',
          }" class="mt-1 max-w-sm flex border rounded-md relative overflow-hidden min-w-0">
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis"
            v-model="Dingtalk.client_id" />
        </div>
        <p class="mt-3 text-gray-600">Client Secret</p>
        <div :class="{
            'border-red-500 hover:border-red-700': Dingtalk.client_secret == '',
            'border-stone-200 hover:border-stone-400': Dingtalk.client_secret != '',
          }" class="mt-1 max-w-sm flex border rounded-md relative overflow-hidden min-w-0">
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis"
      ...
      <div>
        <header class="max-w-sm flex mt-4">
          ...
          <div class="w-full flex justify-end">
            <button :disabled="Microsoft.client_id == '' || Microsoft.client_secret == ''" @click="setMicrosoft"
              class="btn border-0 bg-blue-500 hover:bg-blue-900 disabled:bg-blue-500/60 text-white disabled:text-white/60 h-7 min-h-fit">
              保存
            </button>
          </div>
      ...
      <div>
        <header class="max-w-sm flex mt-4">
          <svg t="1679387527759" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3364"
            width="112" height="28">
            <path
              ...
              fill="#000000" p-id="3365"></path>
          </svg>
          <h3 class="text-xl font-semibold tracking-tight ml-4 min-w-fit">
            GitHub 认证设置
          </h3>
          <div @click="githubHelpShow = true" class="tooltip flex items-center text-stone-300" data-tip="点击查看帮助">
            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2.35" stroke-linecap="round" stroke-linejoin="round" class="ml-1">
              <circle cx="12" cy="12" r="10"></circle>
              <line x1="12" y1="8" x2="12" y2="12"></line>
          ...
          <div class="w-full flex justify-end">
            <button :disabled="Github.client_id == '' || Github.client_secret == ''" @click="setGithub"
              class="btn border-0 bg-blue-500 hover:bg-blue-900 disabled:bg-blue-500/60 text-white disabled:text-white/60 h-7 min-h-fit">
              保存
            </button>
          </div>
      ...
      <div>
        <header class="max-w-sm flex mt-4">
          <svg t="1679449475826" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4669"
            width="112" height="28">
            <path
            ...
            <path
              ...
              fill="#4285F4" p-id="4673"></path>
          </svg>
          <h3 class="text-xl font-semibold tracking-tight ml-4 min-w-fit">
            Google 认证设置
          </h3>
          <div @click="googleHelpShow = true" class="tooltip flex items-center text-stone-300" data-tip="点击查看帮助">
            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2.35" stroke-linecap="round" stroke-linejoin="round" class="ml-1">
              <circle cx="12" cy="12" r="10"></circle>
              <line x1="12" y1="8" x2="12" y2="12"></line>
          ...
          <div class="w-full flex justify-end">
            <button :disabled="Google.client_id == '' || Google.client_secret == ''" @click="setGoogle"
              class="btn border-0 bg-blue-500 hover:bg-blue-900 disabled:bg-blue-500/60 text-white disabled:text-white/60 h-7 min-h-fit">
              保存
            </button>
          </div>
      ...
      <div>
        <header class="max-w-sm flex mt-4">
          <svg t="1679468518353" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1724"
            width="112" height="28">
            <path
              ...
              fill="#0B0B0A" p-id="1725"></path>
          </svg>
          <h3 class="text-xl font-semibold tracking-tight ml-4 min-w-fit">
            Apple 认证设置
          </h3>
          <div @click="appleHelpShow = true" class="tooltip flex items-center text-stone-300" data-tip="点击查看帮助">
            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2.35" stroke-linecap="round" stroke-linejoin="round" class="ml-1">
              <circle cx="12" cy="12" r="10"></circle>
              <line x1="12" y1="8" x2="12" y2="12"></line>
          ...
          <div class="w-full flex justify-end">
            <button :disabled="Apple.client_id == '' ||
              Apple.team_id == '' ||
              Apple.key_id == '' ||
              Apple.private_key == ''
              " @click="setApple"
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="Apple.client_id" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="Apple.team_id" />
        </div>
        ...
        <div :class="{
          ...
          <input class="outline-none py-2 px-3 w-full h-full font-mono text-sm text-ellipsis" v-model="Apple.key_id" />
        </div>
        ...
        <div :class="{
          ...
          <textarea class="textarea outline-none py-2 px-3 w-full h-52 font-mono text-sm text-ellipsis"
  ...
  <!-- 菜单弹出提示框显示 -->
  <Teleport to="body">
    <MicrosoftHelp v-if="microsoftHelpShow" @close="microsoftHelpShow = false"></MicrosoftHelp>
