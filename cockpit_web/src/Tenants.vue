...
<template>
      ...
      <table class="table w-full">
        ...
        <tbody>
          <template v-for="(u, id) in TenantList">
            <tr :id="id" :v-if="u != nil" @mouseenter="mouseOnTenant(u)" @mouseleave="mouseLeaveTenant()"
              ...
              <td class="md:w-1/4 flex-auto md:flex-initial md:shrink-0 w-0 text-ellipsis">
                <div class="flex flex-row items-center">
                  <div
                    ...
                    <svg v-if="u.provider == 'Dingtalk'" class="w-full h-full" viewBox="0 0 16 16" fill="none"
                    ...
                    <svg v-if="u.provider == 'Microsoft'" class="w-full h-full" viewBox="0 0 16 16" fill="none"
                      xmlns="http://www.w3.org/2000/svg">
                      <path d="M0 0H7.57886V7.57886H0V0Z" fill="#F25022"></path>
                      <path d="M0 8.42114H7.57886V16H0V8.42114Z" fill="#00A4EF"></path>
                      <path d="M8.42114 0H16V7.57886H8.42114V0Z" fill="#7FBA00"></path>
                    ...
                    <svg v-if="u.provider == 'Github'" class="w-full h-full" t="1679387527759" viewBox="0 0 1024 1024"
                      version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3364">
                      <path
                    ...
                    <svg v-if="u.provider == 'Google'" class="w-full h-full" t="1679449475826" viewBox="0 0 1024 1024"
                      version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4669">
                      <path
                    ...
                    <svg v-if="u.provider == 'Apple'" class="w-full h-full" t="1679468518353" viewBox="0 0 1024 1024"
                    ...
                    <svg v-if="u.provider == 'WXScan'" class="w-full h-full" xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24">
              ...
              <td class="hidden lg:table-cell md:flex-auto">
                <span>
                  <div class="inline-flex items-center cursor-default">
                    ...
                    <span v-if="u.currentlyConnected" class="text-sm text-gray-600 tooltip tooltip-top" :data-tip="'最近在线于' +
                      new Date(u.lastSeen)
                        .toLocaleString()
                        .replace('/', '年')
                        .replace('/', '月')
                        .replace(' ', '日 ')
                      ">已连接</span>
                    <span v-else class="text-sm text-gray-600 tooltip tooltip-top" :data-tip="new Date(u.lastSeen).getTime() ==
    ...
    : new Date(u.lastSeen)
      ...
      .replace(" ", "日 ")
              ...
              <td class="table-cell justify-end ml-auto md:ml-0 relative w-1/6 lg:w-12">
                <div v-if="(!tenantBtnShow && !tenantMenuShow) || selectTenant.id != u.id"
                  @click="openTenantMenu(u, $event)" class="flex-none w-12 -mt-0.5 relative">
                  <button
  ...
  <!--设备配置菜单显示-->
  <Teleport to="body">
    <TenantMenu v-if="tenantMenuShow" :toleft="btnLeft" :totop="btnTop" :select-tenant="selectTenant"
      @close="closeTenantMenu" @showdialog-removetenant="showRemoveTenant" @showdialog-edittenant="showEditTenant">
    </TenantMenu>
  </Teleport>
  ...
  <Teleport to="body">
    <!-- 移除租户提示框显示 -->
    <RemoveTenant v-if="removeTenantShow" :select-tenant="selectTenant" @close="removeTenantShow = false"
      @confirm-remove="doRemoveTenant">
    </RemoveTenant>

    <!-- 编辑租户提示框显示 -->
    <EditTenant v-if="editTenantShow" :select-tenant="selectTenant" @close="editTenantShow = false"
