...
func (h *Mirage) setLastStateChangeToNow() {
	...
	if err != nil {
		log.Error().
			Caller().
			Err(err).
			Msg("failed to fetch all organizations, failing to update last changed state.")
	}
	...
	for _, org := range orgs {
		...
		h.lastStateChange.Store("o:"+strconv.Itoa(int(org.ID)), now)
	}
}

func (h *Mirage) setOrgLastStateChangeToNow(orgId ...int64) {
	now := time.Now().UTC()
	for _, id := range orgId {
		if h.lastStateChange == nil {
			h.lastStateChange = xsync.NewMapOf[time.Time]()
		}
		h.lastStateChange.Store("o:"+strconv.Itoa(int(id)), now)
	}
}

func (h *Mirage) getLastStateChange() time.Time {
	times := []time.Time{}
...
func (h *Mirage) getOrgLastStateChange(orgId int64) time.Time {
	lastChange, ok := h.lastStateChange.Load("o:" + strconv.Itoa(int(orgId)))
	if ok {
		return lastChange
	} else {
		return time.Now().UTC()
